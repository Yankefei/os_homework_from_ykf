...
vmareaallocmemory(struct proc *p, uint64 dst) {
  ...
  if(growmmapmem(vm->addr_base, vm->len_base, PTE_U | PTE_R | PTE_W) < 0)
    return -1;

  pte_t *pte;
  pte = walk(p->pagetable, vm->addr_base, 0);
  if(pte == 0) {
  ...
  memset((void*)pa0, 0, vm->len_base); // init mem


  ilock(vm->file->ip);

  int ret = readi(vm->file->ip, 0, pa0, vm->offset, vm->len_base);
  // 如果读取的长度小于应该映射的长度，说明文件比较小
  // 需要进行兼容处理
  if (ret != vm->len_base) {
    printf("vmareaallocmemory readi: size: %d, ret: %d\n", vm->len_base, ret);
